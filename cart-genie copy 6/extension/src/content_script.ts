...
// --- RE-INTRODUCED: The robust finder function from your previous working code ---
...
// --- CORRECTED: This function now uses the more reliable finder ---
async function increaseQuantity(card: Element, times: number): Promise<void> {
    const shadowHost = card.querySelector('div[id^="product-card-placeholder-"] > div');
    if (shadowHost && shadowHost.shadowRoot) {
        // Use the robust finder to get the clickable increase button.
...
const performAddToCart = async (item: { name: string, quantity: number }): Promise<void> => {
    ...
    try {
        ...
        if (bestCard) {
            ...
            if (button) {
                button.click();
                // We no longer need a fixed wait here because increaseQuantity now waits itself.
                if (item.quantity > 1) {
