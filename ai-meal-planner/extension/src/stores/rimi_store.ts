...
export class RimiStore extends BaseStore {
    ...
    async searchProduct(query: string): Promise<void> {
        ...
        if (!window.location.href.includes('/search')) {
            window.location.href = `${this.config.url}/e-parduotuve/lt/paieska?query=${encodeURIComponent(query)}`;
            return;
        }
    ...
    private extractProductFromCard(card: Element): Product | null {
        ...
        const nameElement = card.querySelector('.product-name, .product-title, h3, h4');
        ...
        // Extract unit price (usually in a separate element)
        const unitPriceElement = card.querySelector('.unit-price, .price-per-unit');
        let unitPrice = price;
        let unit = 'vnt';
    ...
    async addToCart(product: Product, quantity: number = 1): Promise<void> {
        ...
        for (const button of buttons) {
            ...
            if (text?.includes('krepšel') || text?.includes('cart')) {
        ...
        if (quantity > 1) {
            // Rimi might have a quantity input or +/- buttons
